...
namespace QLVay
{
    public partial class Form2 : Form
    {
        ...
        private void btnGhi_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Bạn chưa điền tên người cho vay");
                txtNgChoVay.Focus();
                return;
            }
            if (txtTienVay.Text != "")
            ...
            {
                MessageBox.Show("Bạn chưa điền số tiền vay");
                txtTienVay.Focus();
                return;
            }
            ...
            if (status == "Insert")
            {
                ...
                string query = "INSERT INTO dbo.QLVay VALUES('" + txtMaVay.Text.Trim() + "','" + txtNgChoVay.Text.Trim() + "','" +
                txtTienVay.Text.Trim() + "','" + dtpNgayVay.Value.ToString("MM/dd/yyyy") + "','" + dtpNgayTra.Value.ToString("MM/dd/yyyy")
                + "','" + txtLaiSuat.Text.Trim() + "','" + cboId.SelectedItem.ToString() + "','" + tienLai + "')";
                SqlCommand cmd = new SqlCommand(query, conn);
            ...
            else
            {
                ...
                string query = "UPDATE dbo.QLVay SET TenNguoiChoVay = '" + txtNgChoVay.Text.Trim() +
                    ...
                    txtLaiSuat.Text.Trim() + "',id = '" + cboId.Text.Trim() + "', TienLaiDuKien = '" + tienLai + "' WHERE MaVay = '" + txtMaVay.Text.Trim() + "'";
                SqlCommand cmd = new SqlCommand(query, conn);
                var result = cmd.ExecuteNonQuery();
                if (result > 0)
                {
